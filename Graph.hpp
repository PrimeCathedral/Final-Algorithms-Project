...
class Graph {
 ...
 public:
  ...
  std::unordered_map<T, std::vector<std::pair<Node<T>, int>>> create_adjacency_list(std::string path_to_csv);

  // Getters
  std::vector<Node<T>> get_vertices() const;
  std::unordered_map<T, std::vector<std::pair<Node<T>, int>>> get_adjacency_list() const;
